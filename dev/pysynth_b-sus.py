...
def make_wav(song,bpm=120,transpose=0,leg_stac=.9,boost=1.1,repeat=0,fn="out.wav", silent=False):
	...
	def render2(a, b, vol, pos, knum, note):
		...
		if note not in list(note_cache.keys()):
			...
			ov = np.exp(-44100./3./decay[int(lf*100)]/44100.)
			new = (( np.sin(sina)
			...
			new *= .5 # np.exp(-x2/decay[int(lf*100)]/44100.)
...
if __name__ == '__main__':
	...
	for n, x in enumerate(song1):
		s = [x]
		make_wav(s, fn = "piano_high_sustain_%02u.wav" % n, transpose=1, bpm=30)
